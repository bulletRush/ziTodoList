...
<script>
	$(document).ready(function() {
        ...
        // Init date picker
        $(function() {
       	    $("[name='employeeBirthday']").datepicker({numberOfMonths: 2, dateFormat: "dd/mm/yy", showWeek: true, firstDay: 1});
       	    $("[name='employeeHireDate']").datepicker({numberOfMouths: 2, dateFormat: "dd/mm/yy", showWeek: true, firstDay: 1});
        });
		...
		$("#newTaskForm").submit(function(e) {
			...
			$.ajax({
				...
				var id = response['new_employee']['id'];
				// Close modal
				$('#newTaskPopup').modal('toggle');
				// Add task to tasklist
				var name = response['new_employee']['name'];
				var birthday = response['new_employee']['birthday'];
				var hireDate = response['new_employee']['hire_date'];
				var probation = response['new_employee']['probation'];
				// Define task
				var task = '<tr id="'+id+'"><td>'+id+'</td><td>'+name+'</td><td>'+birthday+'</td><td>'+hireDate+'</td><td>'+probation+'</td><td><button type="button" class="btn btn-primary editBtn"><span class="glyphicon glyphicon-pencil"></span></button> <button type="button" class="btn btn-danger delBtn"><span class="glyphicon glyphicon-remove"></span></button></td>'+'</tr>';
				if ($('#'+id).length)
                    // Existing task
                    $('#'+id).replaceWith(task);				    					
				else
    ...
    function bindEditButtons(){
        ...
        $(".editBtn").on('click', function(e){
            ...
            $.ajax({
                ...
                $("[name='employeeName']").val(response['employee']['name']);
                $("[name='employeeBirthday']").val(response['employee']['birthday']);
                $("[name='employeeHireDate']").val(response['employee']['hire_date']);
                $("[name='employeeProbation']").val(response['employee']['probation']);
                // Popup task form
                $('#newTaskPopup').modal('show');
            });
        });
    ...
    <nav style="margin-bottom: 100px;" class="navbar navbar-inverse navbar-fixed-top">
        <div class="container-fluid">
            ...
            <div class="navbar-header">
                ...
                <a class="navbar-brand" href="#">表姐的员工管理</a>
            </div>
                    ...
                    <li>
                        <div class='navbar-btn'>
                            ...
                            <button id="newTaskBtn" type="button" class="btn btn-primary">
                                <span class="glyphicon glyphicon glyphicon-plus"></span> 添加新员工
                            </button>
                            <!-- Filters -->
                ...
                <div class="modal-body">
                    <form id="newTaskForm">
                        ...
                        <fieldset class="form-group">
                            ...
                            <input type="text" class="form-control" name="employeeName" placeholder="输入员工姓名">
                        </fieldset>
                        <fieldset class="form-group">
                            <label for="employeeBirthday">生日</label>
                            <input type="text" name="employeeBirthday"></input>
                        </fieldset>
                        <fieldset class="form-group">
                            <span style="float:left">
                                <label for="employeeHireDate">入职时间</label>
                                <input type="text" name="employeeHireDate">
                            </span>        
                        </fieldset>
                        <fieldset class="form-group">
                            <span style="float:left">
                                <label for="employeeProbation">试用期</label>
                                <input type="text" name="employeeProbation">
                            </span>
                        </fieldset>
